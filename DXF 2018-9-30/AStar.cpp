...
VOID AStar::read_map_info(AStarMapInfo &map_info, int type)
{
	...
	临时地址二 = Game::decrypt(偏移地址二 + __BOSS房间X - 24) + 1;
	...
	if (type == 1) {
		map_info.end_room.x = Game::decrypt(偏移地址二 + __BOSS房间X);
		map_info.end_room.y = Game::decrypt(偏移地址二 + __BOSS房间Y);
	}
	else if (type == 2) {
		map_info.end_room.x = Game::decrypt(偏移地址二 + __深渊房间X);
		map_info.end_room.y = Game::decrypt(偏移地址二 + __深渊房间Y);
	}
	//map_info = 地图遍历(模式);
	/*map_info.width = 4;
	map_info.height = 3;
